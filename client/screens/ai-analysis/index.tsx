  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [deviceRes, healthRes, trendRes, userRes, latestAnalysisRes, historyRes, warningsRes, adviceRes] = await Promise.all([
        // 获取设备连接状态
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/bluetooth/devices/${user.boundUserId}`),
        // 获取最新健康数据
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/device-data/bracelet/latest/${user.boundUserId}`),
        // 获取健康趋势数据
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/device-data/health-trend/${user.boundUserId}`),
        // 获取用户标签
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/users/${user.boundUserId}`),
        // 获取最新AI分析
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/ai/latest/${user.boundUserId}?analysis_type=health`),
        // 获取历史分析
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/ai/history/${user.boundUserId}?limit=5`),
        // 获取异常趋势预警
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/health-warning/trend/${user.boundUserId}`),
        // 获取健康建议
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/health-warning/advice/${user.boundUserId}`),
      ]);
        ...
        <View style={styles.container}>
          <View style={styles.header}>
            <TouchableOpacity onPress={() => router.back()} style={styles.backButton}>
              <FontAwesome6 name="arrow-left" size={24} color={colors.textMuted} />
            </TouchableOpacity>
            <ThemedText variant="h3" color={colors.textPrimary}>AI 健康分析</ThemedText>
            <View style={{ width: 40 }} />
          </View>
