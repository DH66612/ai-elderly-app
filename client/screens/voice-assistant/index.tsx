...
import { View, TouchableOpacity, StyleSheet, Alert, ScrollView, Animated, Easing, ActivityIndicator } from 'react-native';
...
export default function VoiceAssistantScreen() {
  ...
  // TTS音频播放函数
  const playTTSSpeech = useCallback(async (text: string) => {
    // 如果正在播放，先停止
    if (soundRef.current) {
      try {
    ...
    try {
      ...
      // 调用后端TTS接口
      const response = await fetch(`${API_BASE_URL}/api/v1/voice-assistant/tts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text,
          speaker: 'xiaoyan', // 晓燕音色，适合老年人
          speechRate: -15, // 稍慢的速度，方便老年人听清
        }),
      });
      ...
      if (data.success && data.audioUri) {
        ...
        console.log('[TTS] TTS生成失败:', data.error);
        setIsSpeaking(false);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!hasInitialized.current && user) {
      ...
      let greeting = '';
      if (hour < 6) {
        greeting = '夜深了，有什么需要帮忙的吗？';
      } else if (hour < 12) {
        greeting = '早上好！今天感觉怎么样？';
      } else if (hour < 14) {
        greeting = '中午好！吃过午饭了吗？';
      } else if (hour < 18) {
        greeting = '下午好！有什么需要帮忙的吗？';
      } else {
        greeting = '晚上好！今天过得怎么样？';
      }
      
      const userName = user.name || '您';
      const welcomeMessage = `${userName}，${greeting}我是您的语音助手，可以帮您查天气、设提醒、查健康数据，或者陪您聊天。`;
  ...
  const handleAIResponse = useCallback(async (userText: string, userId?: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/v1/voice-assistant/chat`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      if (data.success && data.response) {
        ...
        // 自动朗读AI回复
        setTimeout(() => {
          playTTSSpeech(data.response);
        }, 300);
      } else {
        const errorMsg = '抱歉，我没有听清楚，请再说一次。';
        addMessage('ai', errorMsg);
        setTimeout(() => {
          playTTSSpeech(errorMsg);
        }, 300);
      }
    } catch (error: any) {
      console.error('[语音助手] LLM调用失败:', error);
      setMessages(prev => prev.filter(m => m.id !== thinkingMsgId));
      const netErrorMsg = '网络好像有点问题，请检查网络连接后重试。';
      addMessage('ai', netErrorMsg);
      setTimeout(() => {
        playTTSSpeech(netErrorMsg);
      }, 300);
    }
  }, [addMessage, playTTSSpeech]);
  ...
  const handleClear = useCallback(async () => {
    ...
    try {
      await fetch(
        `${API_BASE_URL}/api/v1/voice-assistant/clear-session`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      <View style={styles.container}>
        ...
        <View style={styles.controls}>
          ...
          {isSpeaking && (
            ...
            >
              <FontAwesome6 name="stop-circle" size={32} color="#CF0A2C" />
              <ThemedText variant="title" color="#CF0A2C" style={styles.stopSpeakingText}>停止播放</ThemedText>
            </TouchableOpacity>
          )}
          ...
          <Animated.View style={{ transform: [{ scale: pulseAnim }] }}>
            <TouchableOpacity
              style={[styles.recordButton, isRecording && styles.recordButtonActive, (isRecognizing || isSpeaking) && styles.recordButtonDisabled]}
          ...
          <View style={styles.statusContainer}>
            {isSpeaking ? (
              <>
                <FontAwesome6 name="volume-high" size={20} color="#CF0A2C" />
                <ThemedText variant="bodyMedium" color="#CF0A2C"> 正在朗读...</ThemedText>
              </>
            ) : isRecording ? (
              <>
...
const createStyles = (theme: any) => {
  ...
  return StyleSheet.create({
    ...
    placeholder: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 0 },
    placeholderText: { marginTop: Spacing.lg, textAlign: 'center' },
    controls: { paddingHorizontal: 0, paddingBottom: Spacing['3xl'], gap: Spacing.lg },
    exitButton: {
      flexDirection: 'row',
      alignItems: 'center',
    ...
    stopSpeakingText: { marginLeft: Spacing.sm, fontWeight: '700' },
    recordButton: {
      alignSelf: 'center',
      width: 120,
      height: 120,
