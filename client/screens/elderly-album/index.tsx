...
// 相册存储路径
const ALBUM_DIR = `${documentDirectory}album/`;
...
export default function ElderlyAlbumScreen() {
  ...
  const loadPhotos = async () => {
    try {
      // 确保相册目录存在
      const dirInfo = await FileSystem.getInfoAsync(ALBUM_DIR);
      if (!dirInfo.exists) {
      ...
      // 读取目录内容
      const files = await FileSystem.readDirectoryAsync(ALBUM_DIR);
      const photoItems: PhotoItem[] = [];

      for (const file of files) {
        if (file.endsWith('.jpg') || file.endsWith('.jpeg') || file.endsWith('.png')) {
          const filePath = `${ALBUM_DIR}${file}`;
  ...
  const savePhoto = async (uri: string) => {
    try {
      ...
      const destPath = `${ALBUM_DIR}${fileName}`;
