...
export function useVoiceRecorder(options: UseVoiceRecorderOptions = {}): UseVoiceRecorderReturn {
  ...
  const recognizeAudio = useCallback(async (uri: string): Promise<string | null> => {
    ...
    try {
      ...
      if (Platform.OS === 'web' && uri.startsWith('blob:')) {
        ...
        // 移动端使用 FileSystem 读取
        console.log('[VoiceRecorder] 移动端，使用FileSystem读取文件');
        
        // 先检查文件是否存在和大小
        const fileInfo = await (FileSystem as any).getInfoAsync(uri);
        console.log('[VoiceRecorder] 文件信息:', fileInfo);
        ...
        if (!base64Audio || base64Audio.length < 100) {
          throw new Error('录音数据太短，请长按录音按钮说话');
        }
      }

      console.log('[VoiceRecorder] 发送ASR请求，数据长度:', base64Audio.length);
      
      // 调用后端ASR API
      const apiUrl = getApiBaseUrl();
      const response = await fetch(
        `${apiUrl}/api/v1/asr/recognize`,
        {
          method: 'POST',
          headers: {
      ...
      console.log('[VoiceRecorder] ASR响应:', result.success, result.text || result.error);
  ...
  const stopRecording = useCallback(async (): Promise<string | null> => {
    if (!recordingRef.current) {
      console.log('[VoiceRecorder] 没有正在进行的录音');
      return null;
    }
    ...
    try {
      ...
      console.log('[VoiceRecorder] 录音状态:', status.isRecording, '时长:', status.durationMillis, 'ms');
      ...
      if (uri) {
        console.log('[VoiceRecorder] 录音完成，文件URI:', uri);
        
        // 检查录音时长是否足够（至少500ms）
        if (status.durationMillis < 500) {
          console.log('[VoiceRecorder] 录音时间太短:', status.durationMillis, 'ms');
          setError('录音时间太短，请长按按钮说话');
          return null;
        }
        ...
        console.log('[VoiceRecorder] 未获取到录音文件URI');
        setError('录音文件保存失败');
      }
