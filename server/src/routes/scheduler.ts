...
/**
 * 从详细地址中提取城市名
 * 例如："北京市朝阳区幸福小区3号楼502室" -> "北京"
 */
function extractCityFromAddress(address: string): string {
  if (!address) return '北京';
  
  // 如果已经是简单城市名，直接返回
  if (address.length <= 4 && /[\u4e00-\u9fa5]/.test(address)) {
    return address;
  }
  ...
  const cities = [
    ...
    '杭州', '宁波', '温州', '绍兴', '嘉兴', '金华', '湖州', '丽水', '舟山', '衢州', '台州',
    '南京', '苏州', '无锡', '常州', '南通', '扬州', '镇江', '徐州', '连云港', '淮安', '盐城', '宿迁',
    '成都', '武汉', '西安', '长沙', '郑州', '济南', '青岛', '大连', '沈阳', '哈尔滨', '长春',
    '福州', '厦门', '南昌', '合肥', '昆明', '贵阳', '南宁', '海口', '兰州', '西宁', '银川', '乌鲁木齐',
    '呼和浩特', '拉萨', '太原', '石家庄',
  ];
  ...
  // 尝试提取"省"或"市"前的内容
  const cityMatch = address.match(/([^省]+市|[^市]+区|[^区]+县)/);
  if (cityMatch) {
    let cityName = cityMatch[1];
    // 去掉"市"、"区"、"县"后缀
    cityName = cityName.replace(/[市区县]$/, '');
    // 如果是"北京市朝阳区"的情况，提取"北京"
    if (cityName.includes('省') || cityName.length > 4) {
      const provinceMatch = address.match(/([^省]+省)?([^市]+市)/);
      if (provinceMatch && provinceMatch[2]) {
        return provinceMatch[2].replace('市', '');
      }
    }
    return cityName;
  }
 ...
 * 使用高德地理编码获取城市名
 */
async function getCityByAddress(address: string): Promise<string> {
  if (!AMAP_KEY) {
  ...
  try {
    ...
    if (data.status === '1' && data.geocodes?.[0]) {
      ...
      // 优先返回城市，如果是直辖市则返回省份
      if (geo.city && geo.city.length > 0 && geo.city !== '[]') {
        return geo.city;
      }
      if (geo.province && geo.province.length > 0) {
        // 直辖市直接返回省份名（去掉"市"后缀）
        return geo.province.replace('市', '');
      }
    }
  } catch (error) {
    console.error('[天气] 高德地理编码失败:', error);
  }
  ...
  try {
    ...
    for (const binding of bindings) {
      ...
      const { data: elderlyUser } = await client
        .from('users')
        .select('name, home_address, last_location')
        .eq('id', elderlyId)
        .single();

      const elderlyName = elderlyUser?.name || '老人';

      // 优先使用最后已知位置（GPS坐标），否则回退到家庭地址
      let cityName = '北京';
      let locationSource = 'default';
      
      if (elderlyUser?.last_location?.lat && elderlyUser?.last_location?.lon) {
        // 使用 GPS 坐标获取城市名
        const lat = elderlyUser.last_location.lat;
        const lon = elderlyUser.last_location.lon;
        locationSource = 'gps';
        console.log(`[天气推送] 用户 ${elderlyName} 使用GPS坐标: ${lat}, ${lon}`);
        ...
        if (weatherData) {
          ...
          if (currentCondition && todayForecast) {
            ...
            // 从 nearest_area 获取城市名
            const nearestArea = weatherData.nearest_area?.[0];
            const areaName = nearestArea?.areaName?.[0]?.value || '当前位置';
            const rawCityName = nearestArea?.region?.[0]?.value || areaName;
            cityName = translateCityName(rawCityName);
            
            // 处理天气推送逻辑
            await processWeatherPush(client, guardianId, elderlyId, elderlyName, cityName, temp, weatherDesc, maxTemp, minTemp, notifications);
          }
        }
        continue;
      }
      ...
      cityName = await getCityByAddress(address);
      console.log(`[天气推送] 用户 ${elderlyName} 地址 ${address} -> 城市 ${cityName}`);
 ...
 * 根据坐标获取天气数据
 */
async function fetchWeatherByCoords(lat: number, lon: number): Promise<any> {
  try {
    const response = await fetch(`https://wttr.in/${lat},${lon}?format=j1`);
    const data = await response.json();
    return data;
  } catch (error) {
    console.error('[天气] 根据坐标获取天气失败:', error);
    return null;
  }
}
...
function translateCityName(name: string): string {
  ...
  const cityMap: Record<string, string> = {
    ...
    'xian': '西安', 'chongqing': '重庆', 'tianjin': '天津', 'dongguan': '东莞',
    'foshan': '佛山', 'zhuhai': '珠海', 'zhongshan': '中山', 'huizhou': '惠州',
    'jiangmen': '江门', 'zhaoqing': '肇庆', 'qingyuan': '清远', 'shaoguan': '韶关',
    'shantou': '汕头', 'shanwei': '汕尾', 'yangjiang': '阳江', 'maoming': '茂名',
    'zhanjiang': '湛江', 'meizhou': '梅州', 'heyuan': '河源', 'chaozhou': '潮州',
    'jieyang': '揭阳', 'yunfu': '云浮',
  };
  
  if (cityMap[lowerName]) return cityMap[lowerName];
 ...
 * 处理天气推送逻辑
 */
async function processWeatherPush(
  client: any,
  guardianId: number,
  elderlyId: number,
  elderlyName: string,
  ...
  // 翻译天气描述
  const translatedWeatherDesc = translateWeather(weatherDesc);
  ...
  if (badWeatherKeywords.some(kw => translatedWeatherDesc.includes(kw))) {
    shouldPush = true;
    title = '⚠️ 恶劣天气提醒';
    content = `${elderlyName}所在地区今日天气：${translatedWeatherDesc}，建议老人减少外出，注意安全。`;
  }

  // 每日天气概况（只在早上7点推送一次）
  const hour = new Date().getHours();
  const isDailyWeather = hour === 7;
  
  if (isDailyWeather) {
    shouldPush = true;
    title = '☀️ 今日天气';
    content = `${elderlyName}所在地区${cityName}今日${translatedWeatherDesc}，气温${minTemp}-${maxTemp}°C。祝老人身体健康！`;
  }
