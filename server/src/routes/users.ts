...
async function geocodeAddress(address: string): Promise<string | null> {
  if (!AMAP_KEY) {
    console.log('[Users] 高德API Key未配置，无法自动获取坐标');
    return null;
  }
  ...
  try {
    ...
    console.log('[Users] 地理编码失败:', data.info);
    return null;
  } catch (error) {
    console.error('[Users] 地理编码请求失败:', error);
    return null;
  }
}
...
router.put('/:id', async (req, res) => {
  try {
    ...
    // 自动地理编码：当更新 home_address 且未明确提供 home_location 时
    let resolvedHomeLocation = home_location;
    if (home_address && !home_location) {
      console.log('[Users] 检测到地址更新，自动获取坐标...');
      resolvedHomeLocation = await geocodeAddress(home_address);
      if (resolvedHomeLocation) {
        console.log('[Users] 自动获取坐标成功:', resolvedHomeLocation);
      }
    }
    ...
    if (resolvedHomeLocation !== undefined) updateData.home_location = resolvedHomeLocation;
