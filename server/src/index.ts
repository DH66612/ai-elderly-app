...
function startMedicationReminderScheduler() {
  console.log('[用药提醒] 定时任务已启动（每5分钟检查一次）');
  
  // 每5分钟检查一次（减少服务器压力，同时保证及时性）
  setInterval(async () => {
    try {
      const now = new Date();
...
function startMessagePushScheduler() {
  ...
  // 记录今天已推送的类型
  const pushedToday: Record<string, string> = {};
  ...
  setInterval(async () => {
    ...
    // 天气提醒：早上7点运行（每天一次）
    if (hour === 7 && pushedToday['weather'] !== today) {
      try {
        ...
        if (data.success) {
          pushedToday['weather'] = today;
          if (data.pushed > 0) console.log(`[消息推送] 天气提醒推送了 ${data.pushed} 条`);
        }
      } catch (e) {
    ...
    // 早安推送：早上6点运行（每天一次）
    if (hour === 6 && pushedToday['morning'] !== today) {
      try {
        const res = await fetch(`http://localhost:${port}/api/v1/scheduler/morning-greeting`);
        const data = await res.json() as { success: boolean; pushed: number };
        if (data.success) {
          pushedToday['morning'] = today;
          if (data.pushed > 0) console.log(`[消息推送] 早安推送了 ${data.pushed} 条`);
        }
      } catch (e) {
    ...
    // 晚安推送：晚上21点运行（每天一次）
    if (hour === 21 && pushedToday['evening'] !== today) {
      try {
        const res = await fetch(`http://localhost:${port}/api/v1/scheduler/evening-greeting`);
        const data = await res.json() as { success: boolean; pushed: number };
        if (data.success) {
          pushedToday['evening'] = today;
          if (data.pushed > 0) console.log(`[消息推送] 晚安推送了 ${data.pushed} 条`);
        }
      } catch (e) {
    ...
    // 节日提醒：每天早上8点运行（每天一次）
    if (hour === 8 && pushedToday['festival'] !== today) {
      try {
        const res = await fetch(`http://localhost:${port}/api/v1/scheduler/festival-reminder`);
        const data = await res.json() as { success: boolean; pushed: number };
        if (data.success) {
          pushedToday['festival'] = today;
          if (data.pushed > 0) console.log(`[消息推送] 节日提醒推送了 ${data.pushed} 条`);
        }
      } catch (e) {
    ...
    // 二十四节气提醒：每天早上8点运行（每天一次）
    if (hour === 8 && pushedToday['solarTerm'] !== today) {
      try {
        const res = await fetch(`http://localhost:${port}/api/v1/scheduler/solar-term-reminder`);
        const data = await res.json() as { success: boolean; pushed: number };
        if (data.success) {
          pushedToday['solarTerm'] = today;
          if (data.pushed > 0) console.log(`[消息推送] 节气提醒推送了 ${data.pushed} 条`);
        }
      } catch (e) {
    ...
    // 健康预警：每天上午10点和下午4点检查（每天2次）
    if ((hour === 10 || hour === 16) && pushedToday[`health-${hour}`] !== today) {
      try {
        const res = await fetch(`http://localhost:${port}/api/v1/scheduler/health-warning`);
        const data = await res.json() as { success: boolean; pushed: number };
        if (data.success) {
          pushedToday[`health-${hour}`] = today;
          if (data.pushed > 0) console.log(`[消息推送] 健康预警推送了 ${data.pushed} 条`);
        }
      } catch (e) {
    ...
    // 设备告警：每天上午11点和下午5点检查（每天2次）
    if ((hour === 11 || hour === 17) && pushedToday[`device-${hour}`] !== today) {
      try {
        const res = await fetch(`http://localhost:${port}/api/v1/scheduler/device-alert`);
        const data = await res.json() as { success: boolean; pushed: number };
        if (data.success) {
          pushedToday[`device-${hour}`] = today;
          if (data.pushed > 0) console.log(`[消息推送] 设备告警推送了 ${data.pushed} 条`);
        }
      } catch (e) {
    ...
    // 重置每日推送记录（跨天时）
    if (hour === 0) {
      Object.keys(pushedToday).forEach(key => {
        if (pushedToday[key] !== today) {
          delete pushedToday[key];
        }
      });
    }
  }, 60 * 60 * 1000); // 每小时检查
